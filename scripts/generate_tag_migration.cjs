...
async function generateSQL() {
    console.log('📝 Generating SQL Migration (ALL PALETTES)...');
    ...
    while (hasMore) {
        ...
        allPalettes = allPalettes.concat(data);
        page++;
        console.log(`Fetched page ${page}, total so far: ${allPalettes.length}`);
    }

    console.log(`📊 Total Palettes Found: ${allPalettes.length}`);

    let sqlContent = `-- Update Tags for ${allPalettes.length} palettes\n`;
    sqlContent += `BEGIN;\n`;

    for (const palette of allPalettes) {
        const tags = generateTags(palette.colors);
        const tagsJson = JSON.stringify(tags);
        // Escape single quotes for SQL
        const safeTags = tagsJson.replace(/'/g, "''");

        sqlContent += `UPDATE palettes SET tags = '${safeTags}'::jsonb WHERE id = '${palette.id}';\n`;
    }

    sqlContent += `COMMIT;\n`;

    const outputPath = path.resolve(__dirname, '../update_tags_force.sql');
